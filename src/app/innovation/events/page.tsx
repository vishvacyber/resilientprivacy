    ...
    <div className="min-h-screen bg-gradient-to-br from-[#18181B] via-[#232336] to-[#8B5CF6]/10 py-16 px-4">
      ...
      <div className="max-w-5xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
        <div className="flex gap-2">
          {['All', 'Upcoming', 'Past'].map((tab) => (
            <button
              key={tab}
              className={`px-5 py-2 rounded-full font-semibold text-sm transition-all duration-200 border ${filter === tab ? 'bg-[#8B5CF6] text-white border-[#8B5CF6]' : 'bg-[#232336] text-[#A1A1AA] border-[#232336] hover:bg-[#8B5CF6]/20 hover:text-white'}`}
        ...
        <input
          ...
          className="px-4 py-2 rounded-lg bg-[#232336] border border-[#8B5CF6]/30 text-white placeholder-[#A1A1AA] focus:ring-2 focus:ring-[#8B5CF6] focus:outline-none"
          value={search}
      ...
      <section className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        {filtered.map((event, i) => (
          <div
            key={i}
            className="bg-[#232336] rounded-2xl p-6 border border-[#8B5CF6]/30 shadow-lg flex flex-col justify-between transition-transform duration-200 hover:scale-105 hover:border-[#8B5CF6] group"
          >
            <div>
              <span
                className={`inline-block ${event.type === 'Upcoming' ? 'bg-[#8B5CF6]/20 text-[#8B5CF6]' : 'bg-[#8B5CF6]/10 text-[#8B5CF6]'} font-semibold px-3 py-1 rounded-full text-xs mb-2`}
              >
                {event.type}
              </span>
              ...
              <p className="text-[#A1A1AA] mb-4">{event.desc}</p>
                ...
                {event.speakers &&
                  event.speakers.map((s, idx) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-[#8B5CF6]">{s.title}</span>
                    </div>
                  ))}
              </div>
            </div>
              ...
              {event.type === 'Upcoming' ? (
                <>
                  {getCountdown(event.date) && (
                    <span className="text-xs text-[#8B5CF6] ml-2">
                  ...
                  <a
                    ...
                    className="ml-2 text-xs text-[#8B5CF6] underline"
                  >
                    Add to Calendar
                  </a>
                </>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-5 py-2 rounded-xl font-semibold bg-[#232336] border border-[#8B5CF6] text-[#8B5CF6] hover:bg-[#8B5CF6] hover:text-white transition"
                >
                  {event.action}
                </a>
              ) : null}
            ...
            {event.video && (
              <div className="mt-4 aspect-video rounded-xl overflow-hidden border border-[#8B5CF6]/20">
                <iframe
                  src={event.link}
                  title={event.title}
                  allowFullScreen
      ...
      {showModal && modalEvent && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-[#232336] rounded-2xl p-8 max-w-md w-full shadow-2xl border border-[#8B5CF6]/40 relative">
            <button
              className="absolute top-3 right-3 text-[#8B5CF6] text-xl"
            ...
            <form onSubmit={handleModalSubmit} className="flex flex-col gap-4">
              <input
                ...
                className="px-4 py-2 rounded-lg bg-[#18181B] border border-[#8B5CF6]/30 text-white placeholder-[#8B5CF6] focus:ring-2 focus:ring-[#8B5CF6] focus:outline-none"
                value={email}
